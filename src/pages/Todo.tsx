        ...
        <div className="todo-card">
            ...
            <form onSubmit={handleSubmit}>
                <input
                    ...
                    placeholder="Escribe una tarea..."
                />
                <button type="submit" disabled={isEmptyForm()} > Añadir </button>
            </form>

            <ul>
                {
                    tasks.length === 0 && (
                        <p className="empty">Theres no Task yet </p>
                    )
                }
                <div className="task-list">
                    {tasks.map((task) => (
                        <li key={task.id} className={task.completed ? 'completed' : ''}>
                            ...
                            <button type="button" className="btn-delete" onClick={() => {removeTask(task.id)}} >
                                X
                            </button>
                        </li>
                    ))}
